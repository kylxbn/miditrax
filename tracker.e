...
def main(args: [String]) {
    ...
    if (args.len > 0) {
        ...
        main_menu.add_menu(new Menu("Import XM", 2))
        main_menu.add_menu(new Menu("Exit", 3, MT_CANCEL))
        var cont = true
        var r = ""
        ...
        do {
            ...
            if (r == "Load") { 
                ...
                load_path = run_filechooser("Import XM", "/home", ["*.XM", "*.xm"])
                if (load_path != null) {
                    try {
                        ui_set_app_title("Importing...")
                        mod.load_xm(load_path)
                        ui_set_app_title(NAME+VERSION)
                        mod.edit(true) }
                    catch {
                        ui_set_app_title(NAME+VERSION)
                        run_alert("File error", "The file is either not an XM file, or it is corrupted.", image_from_file("/res/miditrax/error.png"), 4000) } }
                ui_set_screen(main_menu) }
